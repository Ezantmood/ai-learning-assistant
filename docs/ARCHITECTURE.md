...
Kiến trúc theo feature, ít tầng và đủ rõ để sinh viên giải thích. `app/` chỉ điều phối route/UI; mọi lời gọi Supabase nằm trong `src/features/*/api.ts`. React Query quản lý server state, React Hook Form + Zod quản lý form, RLS là ranh giới bảo mật cuối cùng.
...
- `src/lib/env.ts` đọc và kiểm tra hai biến ngay khi khởi tạo; thiếu biến thì báo tên biến, không in giá trị.
