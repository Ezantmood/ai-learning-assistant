...
| FR | File dự kiến | Hàm/điểm kiểm soát dự kiến | Cách kiểm thử | Trạng thái |
|---|---|---|---|---|
| FR-31 | `app/(app)/scan.tsx`; `src/features/scan/{api.ts,schemas.ts,queries.ts,errors.ts}`; `expo-image-picker` (đã cài, không plugin app.json) | `pickScanImage` (`mediaTypes: ['images']`, `result.canceled`/`assets[0]`, `base64: true`); lọc mime nhận png/jpeg/webp/heic/heif, từ chối gif tiếng Việt; testID `scan-pick` | Unit lọc mime + nhánh hủy (mock picker, không gọi mạng) đạt; test tay Expo Go sau CN5-04 | đang làm |
| FR-32 | Cùng FR-31 | `captureScanImage` (`requestCameraPermissionsAsync()` trước `launchCameraAsync()`; `canAskAgain === false` → hướng dẫn Settings; Android `getPendingResultAsync()`); testID `scan-capture` | Unit nhánh từ chối vĩnh viễn + pending Android đạt; test tay Expo Go sau CN5-04 | đang làm |
| FR-33 | `src/lib/ai/transport.ts` (dùng lại, cấm đường gọi thứ hai); `src/lib/ai/models.ts` (hằng số duy nhất, cấm hardcode) | `ocrWithGemini` (prompt text TRƯỚC ảnh, mime `image/jpeg` cố định, JSON schema; OCR riêng `thinkingLevel: minimal`, `mediaResolution: medium`; giữ map 429/5xx, không retry) | Unit payload/parse/map lỗi đạt; OCR smoke thật đạt; chủ dự án đã thấy lượt thành công và 503, cần kiểm tay lại sau tối ưu | đang làm |
