...
| FR-09 | `app/(app)/documents/[id].tsx`; `src/features/documents/{api.ts,queries.ts}` | `getDocument`; `getDocumentUrl` (signed URL TTL 3600s, cache 55 phút); nút “Mở tài liệu” (`Linking.openURL` + `canOpenURL`, tiện ích ngoài FR, không WebView) | G2: test tay chi tiết + mở xem ở light/dark (checklist CN2-G2) | Đang làm |
| FR-10 | Cùng màn chi tiết; `src/features/documents/api.ts` | `renameDocument` (chỉ đổi `display_name`, không đổi object) | G2: unit test 26 mới PASS (payload update chỉ `display_name`; tên rỗng/quá dài bị chặn, không gọi update); test tay tên sai/giữ tên cũ | Đang làm |
| FR-11 | Cùng màn chi tiết; Storage policy `documents_delete_own` | `deleteDocument` (storage trước, DB sau, có dialog) | G2: unit test thứ tự storage→DB + nhánh storage-lỗi-giữ-bản-ghi + nhánh DB-fail→`DocumentDeletePartialError` (báo rõ + thử lại); test tay hủy dialog/xóa thật | Đang làm |
| FR-12 | `app/(app)/subjects/index.tsx`; `app/(app)/documents/[id].tsx` (gán); bảng `subjects`; `documents.subject_id ON DELETE SET NULL` | `listSubjects`; `createSubject` (guard trần 30); `renameSubject`; `deleteSubject`; `assignDocumentSubject` (chỉ ở chi tiết); `countDocumentsInSubject` (cảnh báo trước xóa) | G2: unit test schema 1–60/trùng tên/trần 30 + lọc `filterDocumentsLocal`; test tay xóa môn đang có tài liệu → “Chưa phân loại” | Đang làm |
| FR-13 | Bảng `documents` (`extracted_text`, `extraction_status`) | Hạ tầng ở CN2 (`pending`/`unsupported`), thực thi trích xuất ở CN3 | CN2 chỉ làm hạ tầng hai cột (G1 gắn status khi upload, G2 hiển thị nhãn tiếng Việt ở chi tiết); CN3 gọi AI trích nội dung rồi đổ vào — FR-13 tách đôi, không phải bỏ sót | Đang làm |
