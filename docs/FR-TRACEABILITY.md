...
| FR-33 | `src/lib/ai/transport.ts` (dùng lại, cấm đường gọi thứ hai); `src/lib/ai/models.ts` (hằng số duy nhất, cấm hardcode) | `ocrWithGemini` (prompt text TRƯỚC ảnh, mime `image/jpeg` cố định, `responseMimeType` + `responseSchema`; giữ map 429/5xx CN3, không retry) | Unit payload/parse/map lỗi (mock fetch, không gọi mạng) đạt; test tay OCR thật sau CN5-04 | đang làm |
